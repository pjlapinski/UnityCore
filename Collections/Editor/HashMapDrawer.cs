...
public static class KvpExtension {
  public static KeyValue<TKey, TValue> ToKeyValue<TKey, TValue>(KeyValuePair<TKey, TValue> kvp) => new(kvp.Key, kvp.Value);
}
...
public class HashMapDrawer : PropertyDrawer {
  ...
  private SerializedProperty GetModifiedElements(SerializedProperty property) {
    ...
    foreach (var val in obj) {
      valuesField.GetArrayElementAtIndex(i++).boxedValue = KvpExtension.ToKeyValue(val);
    }
    return valuesField;
  }
}
