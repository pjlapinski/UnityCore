...
namespace PJL.Data.Editor
{
    ...
    public class UuidDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (GUI.Button(copyRect, new GUIContent("Copy"))) EditorGUIUtility.systemCopyBuffer = value.ToString();
            ...
            if (GUI.Button(newRect, new GUIContent("New"))) property.boxedValue = Uuid.NewUuid();
            x += btnWidth + HorizontalSpacing;

            var emptyRect = new Rect(x, position.y, btnWidth, EditorGUIUtility.singleLineHeight);
            if (GUI.Button(emptyRect, new GUIContent("Empty"))) property.boxedValue = Uuid.Empty;
