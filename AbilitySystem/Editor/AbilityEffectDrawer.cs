...
namespace PJL.AbilitySystem.Editor
{
    ...
    public class AbilityEffectDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (EditorGUI.DropdownButton(typeRect, new GUIContent(typeName ?? "Select AbilityEffect type"), FocusType.Keyboard))
            {
                var menu = new GenericMenu();
                if (_inheritors == null || _inheritors.Count == 0)
                {
                    menu.AddDisabledItem(new GUIContent("No AbilityEffects available"));
                }
                else
                {
        ...
        private void BuildInheritorsMap() =>
            _inheritors = AppDomain
                ...
                .Where(type => !type.IsAbstract && typeof(AbilityEffect).IsAssignableFrom(type))
