...
namespace PJL.DependencyInjection
{
    public class ServiceCollection
    {
        ...
        private object[] GetInitializationMethodData(ServiceDescriptor descriptor, out MethodInfo methodInfo)
        {
            ...
            if (initParams == null)
            {
                Debug.LogError($"Could not find a matching initialization method with name \"{initMethodName}\" for the service of type {descriptor.ServiceType.Name}.");
                methodInfo = null;
                return null;
            }
            ...
            for (var i = 0; i < initParams.Length; i++)
            {
                ...
                if (attr == null)
                {
                    Debug.LogError($"Could not resolve an initialization dependency for the service of type {descriptor.ServiceType.Name}. {param.Name} was null");
                    methodInfo = null;
                    return null;
                }
        ...
        private void RegisterService<T, TService>(in object implementation, ServiceCreationPolicy serviceCreationPolicy, ServiceLifetime lifetime, string resourcePath = null) where TService : T
        {
            var serviceType = typeof(T);
            if (_services.ContainsKey(serviceType))
                throw new DuplicateNameException($"The container already has a definition of the {serviceType.Name} service.");
        ...
        /// <typeparam name="T">The type the service will register as.</typeparam>
        /// <typeparam name="TService">The type of the implementation of the service, which implements the service type.</typeparam>
        public void RegisterSingleton<T, TService>(in object implementation, ServiceCreationPolicy serviceCreationPolicy) where TService : T 
        ...
        public object GetService(Type serviceType)
        {
            ...
            return descriptor.CreationPolicy switch
            {
                ServiceCreationPolicy.Self when descriptor.Lifetime == ServiceLifetime.Singleton => 
                    throw new NullReferenceException($"There is no implementation for the service of type {descriptor.ServiceType.Name} with creation policy 'Self'."),
                ServiceCreationPolicy.Self when descriptor.Lifetime == ServiceLifetime.Transient => 
                    throw new ArgumentException($"There cannot be a transient implementation for the service of type {descriptor.ServiceType.Name} with creation policy 'Self'."),
                ServiceCreationPolicy.NewInstance when (isMono || isScriptable || descriptor.ImplementationType.IsAbstract || descriptor.ImplementationType.IsInterface) => 
                    throw new ArgumentException($"Trying to make a new instance of service {descriptor.ServiceType.Name}, but it's implementation type does not allow for 'new' instantiation."),
                ServiceCreationPolicy.NewInstance => 
                    GetServiceByNewInstance(descriptor),
                ServiceCreationPolicy.NewGameObject when !isMono || isScriptable || descriptor.ImplementationType.IsAbstract || descriptor.ImplementationType.IsInterface => 
                    throw new ArgumentException($"Trying to make a new instance of service {descriptor.ServiceType.Name}, but it's implementation type does not allow for Component instantiation."),
                ServiceCreationPolicy.NewGameObject => 
                    GetServiceByNewGameObject(descriptor),
                ServiceCreationPolicy.Find when !isMono || isScriptable || descriptor.ImplementationType.IsAbstract || descriptor.ImplementationType.IsInterface => 
                    throw new ArgumentException($"Trying to Object.Find an instance of service {descriptor.ServiceType.Name}, but it's implementation type does not allow for Component instantiation."),
                ServiceCreationPolicy.Find when descriptor.Lifetime == ServiceLifetime.Transient => 
                    throw new ArgumentException($"Trying to Object.Find an instance of service {descriptor.ServiceType.Name}, but it's lifetime is Transient."),
                ServiceCreationPolicy.Find => 
                    GetServiceByFind(descriptor),
                ServiceCreationPolicy.Resource when !isScriptable =>
                    throw new ArgumentException($"Trying to Resources.Load an instance of service {descriptor.ServiceType.Name}, but it's implementation is not a ScriptableObject."),
                ServiceCreationPolicy.Resource when string.IsNullOrWhiteSpace(descriptor.ResourcePath) =>
                    throw new ArgumentException($"Trying to Resources.Load an instance of service {descriptor.ServiceType.Name}, but no path was given to the resource."),
